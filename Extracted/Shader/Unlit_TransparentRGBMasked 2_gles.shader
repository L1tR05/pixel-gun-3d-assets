...
//
///////////////////////////////////////////
Shader "Unlit/TransparentRGBMasked 2" {
Properties {
...
void main ()
{
  ...
  highp vec2 factor_3;
  highp vec2 tmpvar_4;
  tmpvar_4 = ((vec2(1.0, 1.0) - abs(xlv_TEXCOORD2.xy)) * _ClipArgs0);
  factor_3 = ((vec2(1.0, 1.0) - abs(xlv_TEXCOORD2.zw)) * _ClipArgs1.xy);
  lowp vec4 tmpvar_5;
  tmpvar_5 = (texture2D (_MainTex, xlv_TEXCOORD0) * xlv_COLOR);
  col_2 = tmpvar_5;
  highp float tmpvar_6;
  tmpvar_6 = clamp (min (min (tmpvar_4.x, tmpvar_4.y), min (factor_3.x, factor_3.y)), 0.0, 1.0);
  col_2.w = (col_2.w * tmpvar_6);
  lowp float tmpvar_7;
  tmpvar_7 = dot (texture2D (_Mask, xlv_TEXCOORD1).xyz, vec3(0.5, 0.7, 0.3));
